...
const DataTable: React.FunctionComponent<IDataTableProps> = ({data,updateReload}) => {
            ...
            return (
                <tr key={item._id} className="border-b text-white bg-gray-600 hover:bg-white hover:text-gray-800">
                    <td className="px-6 py-3 break-words">
                        <Link to={item.fullUrl} target="_blank" rel="noreferer noopener" >{item.fullUrl}</Link>
                    </td>
                    <td className="px-6 py-3 break-words">
                        <Link to={`${serverUrl}/shortUrl/${item.fullUrl}`} target="_blank" rel="noreferer noopener" >{item.shortUrl}</Link>
                    </td>
                    ...
                    <td className="px-6 py-3">
                        <div className="flex content-center">
                            ...
                            <div className="px-2 cursor-pointer" onClick={()=>deleteUrl(item._id)}>
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="size-6 cursor-pointer">
    ...
    const copyToClipboard = async(shortUrl:string) => {
        try {
            await navigator.clipboard.writeText(`${serverUrl}/shortUrl/${shortUrl}`) ;
            alert(`URL copied : ${serverUrl}/shortUrl/${shortUrl}`) ;
        } catch (error) {
            console.log(error);
        }
    }
